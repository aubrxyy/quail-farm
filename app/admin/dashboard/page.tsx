...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Calculate summary from orders if finance data is not available
      ...
      if (financeData) {
        setDashboardData({
          summary: {
            totalRevenue: financeData.summary.totalRevenue || totalRevenue,
            totalExpenses: financeData.summary.totalExpenses || 0,
            netProfit: financeData.summary.netProfit || totalRevenue,
            totalOrders: financeData.summary.totalOrders || totalOrders,
            activeEmployees: financeData.summary.activeEmployees || 0,
        ...
        // Fallback when finance API is not available
        setDashboardData({
          summary: {
            totalRevenue,
            totalExpenses: 0,
  ...
  return (
    <div className="flex text-black bg-bright-egg-white min-h-screen">
      <div className="px-8 pt-4 flex flex-col gap-y-6 w-full">
        ...
        {/* Order Map with Real Data */}
        <OrderMap orders={filteredOrders.map(order => ({
          id: order.id,
          customerName: order.customerName,
          customerAddress: order.customerAddress,
          lat: order.address?.latitude || -6.5954, // fallback coordinates for Bogor
          lng: order.address?.longitude || 106.8162,
          status: order.status,
                ...
                dashboardData.recentOrders.map((order: any) => (
                  <div key={order.id} className="flex justify-between items-center p-3 border border-gray-100 rounded-lg hover:bg-gray-50">
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-gray-400">{formatDate(order.orderDate || order.createdAt)}</div>
                    </div>
